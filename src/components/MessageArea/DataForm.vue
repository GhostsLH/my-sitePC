  ...
  <form id="data-form-container" ref="form" @submit.prevent="handleSubmit" class="data-form-container">
    <div class="form-item">
      <div class="input-area">
        ...
        />
        <span class="tip"
          >{{ formData.nickname ? formData.nickname.length : 0 }}/10</span
        >
      </div>
    ...
    <div class="form-item">
      <div class="text-area">
        ...
        ></textarea>
        <span class="tip"
          >{{ formData.content ? formData.content.length : 0 }}/300</span
        >
      </div>
...
export default {
  ...
  methods: {
    handleSubmit() {
      ...
      this.isSubmiting = true; //正在提交中，防止重复点击
      this.$emit("submit", this.formData, (successMsg) => {
        this.$showMessage({
          ...
          callback: () => {
            this.isSubmiting = false; //提交成功，��复提交状态
